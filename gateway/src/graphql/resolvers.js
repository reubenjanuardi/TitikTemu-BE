...
const resolvers = {
  ...
  Query: {
    ...
    me: async (_, __, context) => {
      ...
      try {
        const response = await axios.get(
          ...
          { headers: { Authorization: `Bearer ${context.req.headers.authorization?.split(' ')[1]}` } }
        );
    ...
    eventParticipants: async (_, { eventId }, context) => {
      ...
      try {
        const response = await axios.get(
          `${config.services.eventServiceUrl}/events/${eventId}/participants`,
    ...
    venue: async (_, { id }) => {
      try {
        const response = await axios.get(
          `${config.services.venueServiceUrl}/venues/${id}`
        );
        return response.data.data;
      } catch (error) {
        if (error.response?.status === 404) return null;
    ...
    venueAvailability: async (_, { venueId, date }, context) => {
      requireAuth(context);
      
      try {
        const response = await axios.get(
          `${config.services.venueServiceUrl}/venues/${venueId}/availability`,
          { 
            params: { date },
            headers: createAuthHeaders(context.user)
          }
    ...
    logisticsByCategory: async (_, { category }) => {
      try {
        const response = await axios.get(
          `${config.services.venueServiceUrl}/venues/logistics/${category}`
        );
        return response.data.data || [];
      } catch (error) {
        handleServiceError(error, 'Venue Service');
    ...
    eventAttendance: async (_, { eventId }, context) => {
      ...
      try {
        const response = await axios.get(
          `${config.services.attendanceServiceUrl}/attendance/event/${eventId}`,
          { headers: createAuthHeaders(context.user) }
        );
    ...
    attendanceStats: async (_, { eventId }, context) => {
      ...
      try {
        const response = await axios.get(
          `${config.services.attendanceServiceUrl}/attendance/stats/${eventId}`,
          { headers: createAuthHeaders(context.user) }
        );
        return response.data.data;
      } catch (error) {
  ...
  Mutation: {
    ...
    registerForEvent: async (_, { eventId }, context) => {
      ...
      try {
        const response = await axios.post(
          `${config.services.eventServiceUrl}/events/${eventId}/register`,
          {},
          { headers: createAuthHeaders(context.user) }
        );
        return response.data.data;
